...
public class DeptWindow extends JFrame {
	...
	/** Completa el método showDialog de DeptWindow.java para que llame a los métodos del servicio 
	 * create o update n función de si se ha creado o editado el departamento. 
	 * Han de controlarse las excepciones y han de mostrarse por lo menos los siguientes 
	 * mensajes con el método addMensaje: 
	 * 1."El departamento se ha creado/actualizado correctamente" 
	 * 2."Ya existe un departamento con ese id. No se ha podido crear." 
	 * 3. "Ha ocurrido un error y no se ha podido crear el departamento" 
	 */	
	private void showDialog() {
		...
		if (departamentoACrear != null) {
			...
			if (createDialog.getTipo() == TIPO_EDICION.CREAR) {
				...
				//comprobación de si ya existe
				if (departamentoServicio.exists(departamentoACrear.getDeptno())) {
					System.out.println("Ya existe un departamento con ese id. No se ha podido crear. " + departamentoACrear.getDeptno());					
					addMensaje(true, "Ya existe un departamento con ese id. No se ha podido crear.");
				} else {
					try {
						departamentoServicio.create(departamentoACrear);
						System.out.println("El departamento se ha creado/actualizado correctamente" + departamentoACrear.getDname());
						addMensaje(true, "El departamento se ha creado correctamente");
						
					} catch (DuplicateInstanceException e) {
						addMensaje(true, "\"Ha ocurrido un error. Departamento duplicado. ");
				...
				if (departamentoACrear != null) {
					try {
						...
						if (exito) {
							addMensaje(true, "\"Se ha actualizado correctamente el departamento con id: " + departamentoACrear.getDeptno());
							getAllDepartamentos();
						}
					
					} catch (Exception ex) {
						addMensaje(true, "\"Ha ocurrido un error y no se ha podido modificar el departamento. ");
						System.out.println("Exception: " + ex.getMessage());
						ex.printStackTrace();
					}
				}				
